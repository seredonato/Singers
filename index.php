...
<form class="form-inline my-2 my-lg-0" method="get" id="form-search" action="buscarCantante.php">
    <input class="form-control mr-sm-2" id="search" name="search" type="search"
           placeholder="Ingrese nombre, género o número" aria-label="Search">
    <button class="btn btn-info my-2 boton-search" type="submit">Search Singer</button>
</form>
...
<?php
$eliminado = isset($_GET["eliminarCantante"]) ? $_GET["eliminarCantante"] : false;
$modificado = isset($_GET["modificarCantante"]) ? $_GET["modificarCantante"] : false;
$idExistente = isset($_GET["idExistente"]) ? $_GET["idExistente"] : false;
$archivo = isset($_GET["archivoSubidoError"]) ? $_GET["archivoSubidoError"] : false;
$creado = isset($_GET["crearCantante"]) ? $_GET["crearCantante"] : false;
$usuario = isset($_GET["datosIncorrectos"]) ? $_GET["datosIncorrectos"] : false;

if ($eliminado == true) {
    echo '
            <div class="container text-center">
            <h4>El Cantante ha sido eliminado correctamente.</h4>
            </div>';
} elseif ($usuario == true) {
    echo '
            <div class="container text-center">
...
} elseif ($modificado == true) {
    echo '
            <div class="container text-center">
            <h4>La modificación se realizó con éxito.</h4>
            </div>';
} elseif ($idExistente == true) {
    echo '
            <div class="container text-center">
            <h4>El número de Cantante que intenta asignar ya existe.</h4>
            </div>';
} elseif ($archivo == true) {
    echo '
            <div class="container text-center">
    ...
    echo '
            <div class="container text-center">
            <h4>El cantante ha sido creado exitosamente.</h4>
            </div>';
}
?>
